    ...
    <section>
        <div class="row py-5">
            <div class="col-lg-10">
                <div class="card mb-4">
                    <div class="card-body">
                        <form method="post" action="{{ url('/dashboard/transactions/' . $transaction->id) }}">
                            @method('put')
                            @csrf
                            <div class="col-sm-9">
                                        ...
                                        @foreach ($books as $book)
                                            @if ($transaction->book_id == $book->id)
                                                <option value="{{ $book->id }}" selected>{{ $book->judul }} |
                                                    Umur : {{ $book->eksemplar }}</option>
                                            @else
                                                <option value="{{ $book->id }}">{{ $book->judul }} | Umur : {{ $book->eksemplar }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-sm-3">
                                    <p class="mb-0">Proses Pencarian</p>
                                </div>
                                <div class="col-sm-9">
                            ...
                            @if ($transaction->status == 'PEMINJAMAN')
                                ...
                                <hr>
                                <button class="btn btn-primary" type="submit">Ubah Data Kehilangan</button>
                            @else
                                <div class="row">
                                    <div class="col-sm-3">
                                        <p class="mb-0">Status</p>
                                ...
                                <div class="row" id="IfPinjam">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-3">
                                            <p class="mb-0">Tanggal Ditemukan</p>
                                        </div>
                                        <div class="col-sm-9">
                                            <input type="date" class="form-control" id="kembali"
                                                name="tgl_pengembalian" value="<?php echo date('Y-m-d'); ?>"
                                                min="<?php
                                                echo date('Y-m-d', strtotime($transaction->tgl_pinjam . ' + 5 day')); ?>" onchange="PinjamFunction()">
                                        </div>
                                    </div><hr>
                                    {{-- <hr>
                                    <div class="row">
